...
struct ReadShapefilePolygonTest : public GdfTest {
  bool check_polygon(const cuspatial::polygons<double>& h_polygon,
                     const gdf_column& f_pos,
                     const gdf_column& r_pos,
                     const gdf_column& poly_x,
                     const gdf_column& poly_y)
  {
    CUDF_EXPECTS(h_polygon.num_feature == (uint32_t)f_pos.size,
                 "number of features/polygons mismatches expected");
    CUDF_EXPECTS(h_polygon.num_ring == (uint32_t)r_pos.size, "number of rings mismatches expected");
    CUDF_EXPECTS(h_polygon.num_vertex == (uint32_t)poly_x.size,
                 "number of vertices mismatches expected");
    ...
    EXPECT_EQ(cudaMemcpy(h_f_pos.data(),
                         f_pos.data,
    ...
    EXPECT_EQ(
      cudaMemcpy(
        h_r_pos.data(), r_pos.data, h_polygon.num_ring * sizeof(uint32_t), cudaMemcpyDeviceToHost),
      cudaSuccess);
    ...
    EXPECT_EQ(
      cudaMemcpy(
        h_x.data(), poly_x.data, h_polygon.num_vertex * sizeof(double), cudaMemcpyDeviceToHost),
      cudaSuccess);
    for (size_t i = 0; i < h_polygon.num_vertex; i++) EXPECT_NEAR(h_polygon.x[i], h_x[i], 1e-9);

    std::vector<double> h_y(h_polygon.num_vertex);
    EXPECT_EQ(
      cudaMemcpy(
        h_y.data(), poly_y.data, h_polygon.num_vertex * sizeof(double), cudaMemcpyDeviceToHost),
      cudaSuccess);
    for (size_t i = 0; i < h_polygon.num_vertex; i++) EXPECT_NEAR(h_polygon.y[i], h_y[i], 1e-9);
    return true;
  }
};
...
TEST_F(ReadShapefilePolygonTest, testNonExist)
{
  ...
  std::string shape_filename =
    std::string(env_p) + std::string("/test_fixtures/shapefiles/non_exist.shp");
...
TEST_F(ReadShapefilePolygonTest, testZero)
{
  ...
  CUDF_EXPECTS(env_p != NULL, "CUSPATIAL_HOME environmental variable must be set");
  std::string shape_filename =
    std::string(env_p) + std::string("/test_fixtures/shapefiles/empty_poly.shp");
  std::cout << "Using shapefile " << shape_filename << std::endl;
...
TEST_F(ReadShapefilePolygonTest, testOne)
{
  ...
  CUDF_EXPECTS(env_p != NULL, "CUSPATIAL_HOME environmental variable must be set");
  std::string shape_filename =
    std::string(env_p) + std::string("/test_fixtures/shapefiles/one_poly.shp");
  std::cout << "Using shapefile " << shape_filename << std::endl;
  ...
  gdf_column f_pos, r_pos, poly_x, poly_y;
  cuspatial::read_polygon_shapefile(shape_filename.c_str(), &f_pos, &r_pos, &poly_x, &poly_y);
  CUDF_EXPECTS(this->check_polygon(h_polygon, f_pos, r_pos, poly_x, poly_y),
               "polygon readout mismatches expected");
}
...
TEST_F(ReadShapefilePolygonTest, testTwo)
{
  ...
  CUDF_EXPECTS(env_p != NULL, "CUSPATIAL_HOME environmental variable must be set");
  std::string shape_filename =
    std::string(env_p) + std::string("/test_fixtures/shapefiles/two_polys.shp");
  std::cout << "Using shapefile " << shape_filename << std::endl;
