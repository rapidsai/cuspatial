...
TEST_F(CoordinateTransToy, coordinatetranstest)
{
  ...
  std::copy(
    point_lon_vec.begin(), point_lon_vec.end(), std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
  std::cout << "lat:" << std::endl;
  std::copy(
    point_lat_vec.begin(), point_lat_vec.end(), std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
  ...
  std::pair<gdf_column, gdf_column> res_pair =
    cuspatial::lonlat_to_coord(x0, y0, *(point_lon_wrapp.get()), *(point_lat_wrapp.get()));
