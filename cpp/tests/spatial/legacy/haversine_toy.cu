...
TEST_F(HaversineToy, haversinetest)
{
  ...
  CUDF_EXPECTS(
    h_pair_x1 != nullptr && h_pair_y1 != nullptr && h_pair_x2 != nullptr && h_pair_y2 != nullptr,
  ...
  cudf::test::column_wrapper<double> point_x1_wrapp{
    std::vector<double>(h_pair_x1, h_pair_x1 + num_point * num_point)};
  cudf::test::column_wrapper<double> point_y1_wrapp{
    std::vector<double>(h_pair_y1, h_pair_y1 + num_point * num_point)};
  cudf::test::column_wrapper<double> point_x2_wrapp{
    std::vector<double>(h_pair_x2, h_pair_x2 + num_point * num_point)};
  cudf::test::column_wrapper<double> point_y2_wrapp{
    std::vector<double>(h_pair_y2, h_pair_y2 + num_point * num_point)};

  gdf_column dist = cuspatial::haversine_distance(*(point_x1_wrapp.get()),
                                                  *(point_y1_wrapp.get()),
                                                  *(point_x2_wrapp.get()),
                                                  *(point_y2_wrapp.get()));
