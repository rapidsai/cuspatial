...
TEST_F(BoundingBoxTest, test_one)
{
  ...
  std::cout << "num_poly=" << num_poly << ",num_ring=" << num_ring << ",num_vertex=" << num_vertex
            << std::endl;
  ...
  std::unique_ptr<cudf::column> fpos_col = cudf::make_numeric_column(
    cudf::data_type{cudf::type_id::INT32}, num_poly, cudf::mask_state::UNALLOCATED, stream, mr);
  uint32_t *d_p_fpos =
    cudf::mutable_column_device_view::create(fpos_col->mutable_view(), stream)->data<uint32_t>();
  assert(d_p_fpos != nullptr);
  CUDA_TRY(cudaMemcpy(d_p_fpos, poly_fpos, num_poly * sizeof(uint32_t), cudaMemcpyHostToDevice));

  std::unique_ptr<cudf::column> rpos_col = cudf::make_numeric_column(
    cudf::data_type{cudf::type_id::INT32}, num_ring, cudf::mask_state::UNALLOCATED, stream, mr);
  uint32_t *d_p_rpos =
    cudf::mutable_column_device_view::create(rpos_col->mutable_view(), stream)->data<uint32_t>();
  assert(d_p_rpos != nullptr);
  CUDA_TRY(cudaMemcpy(d_p_rpos, poly_rpos, num_ring * sizeof(uint32_t), cudaMemcpyHostToDevice));

  std::unique_ptr<cudf::column> x_col = cudf::make_numeric_column(
    cudf::data_type{cudf::type_id::FLOAT64}, num_vertex, cudf::mask_state::UNALLOCATED, stream, mr);
  double *d_p_x =
    cudf::mutable_column_device_view::create(x_col->mutable_view(), stream)->data<double>();
  assert(d_p_x != nullptr);
  CUDA_TRY(cudaMemcpy(d_p_x, poly_x, num_vertex * sizeof(double), cudaMemcpyHostToDevice));

  std::unique_ptr<cudf::column> y_col = cudf::make_numeric_column(
    cudf::data_type{cudf::type_id::FLOAT64}, num_vertex, cudf::mask_state::UNALLOCATED, stream, mr);
  double *d_p_y =
    cudf::mutable_column_device_view::create(y_col->mutable_view(), stream)->data<double>();
  assert(d_p_y != nullptr);
  CUDA_TRY(cudaMemcpy(d_p_y, poly_y, num_vertex * sizeof(double), cudaMemcpyHostToDevice));

  // GPU computation
  std::unique_ptr<cudf::experimental::table> bbox_tbl =
    cuspatial::polygon_bbox(*fpos_col, *rpos_col, *x_col, *y_col);

  CUDF_EXPECTS(bbox_tbl->view().num_columns() == 4, "bbox table must have 4 columns");
  CUDF_EXPECTS((uint32_t)(bbox_tbl->num_rows()) == num_poly,
  ...
  EXPECT_EQ(cudaMemcpy(h_rx1, d_rx1, num_poly * sizeof(double), cudaMemcpyDeviceToHost),
            cudaSuccess);
  EXPECT_EQ(cudaMemcpy(h_ry1, d_ry1, num_poly * sizeof(double), cudaMemcpyDeviceToHost),
            cudaSuccess);
  EXPECT_EQ(cudaMemcpy(h_rx2, d_rx2, num_poly * sizeof(double), cudaMemcpyDeviceToHost),
            cudaSuccess);
  EXPECT_EQ(cudaMemcpy(h_ry2, d_ry2, num_poly * sizeof(double), cudaMemcpyDeviceToHost),
            cudaSuccess);
...
TEST_F(BoundingBoxTest, test_small)
{
  ...
  CUDF_EXPECTS(num_vertex = poly_rpos[num_ring - 1],
               "# of vertex should be the same as the last element of the ring array");
  ...
  if (0) {
    ...
    thrust::copy(x1_ptr, x1_ptr + num_poly, std::ostream_iterator<double>(std::cout, " "));
    std::cout << std::endl;

    std::cout << "y1:" << std::endl;
    thrust::device_ptr<const double> y1_ptr = thrust::device_pointer_cast(d_ry1);
    thrust::copy(y1_ptr, y1_ptr + num_poly, std::ostream_iterator<double>(std::cout, " "));
    std::cout << std::endl;

    std::cout << "x2:" << std::endl;
    thrust::device_ptr<const double> x2_ptr = thrust::device_pointer_cast(d_rx2);
    thrust::copy(x2_ptr, x2_ptr + num_poly, std::ostream_iterator<double>(std::cout, " "));
    std::cout << std::endl;

    std::cout << "y2:" << std::endl;
    thrust::device_ptr<const double> y2_ptr = thrust::device_pointer_cast(d_ry2);
    thrust::copy(y2_ptr, y2_ptr + num_poly, std::ostream_iterator<double>(std::cout, " "));
    std::cout << std::endl;
  }
