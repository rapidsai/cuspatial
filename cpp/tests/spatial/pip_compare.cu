...
struct PIPCompare : public GdfTest {
  ...
  int set_initialize(const char *poly_filename, const char *point_filename)
  {
    ...
    CUDF_EXPECTS(h_polygon.num_feature > 0 && h_polygon.num_ring > 0,
                 "invalid # of polygons/rings");
    CUDF_EXPECTS(h_polygon.num_feature <= h_polygon.num_ring,
                 "a h_polygon must have at least one ring_length");
    CUDF_EXPECTS(h_polygon.x != nullptr && h_polygon.y != nullptr,
                 "h_polygon vertex h_point_x/h_point_y array can not be nullptr");
    ...
    std::partial_sum(h_polygon.group_length,
                     h_polygon.group_length + h_polygon.num_group,
                     h_polygon.group_position,
                     std::plus<uint32_t>());
    std::partial_sum(h_polygon.feature_length,
                     h_polygon.feature_length + h_polygon.num_feature,
                     h_polygon.feature_position,
                     std::plus<uint32_t>());
    std::partial_sum(h_polygon.ring_length,
                     h_polygon.ring_length + h_polygon.num_ring,
                     h_polygon.ring_position,
                     std::plus<uint32_t>());
    gettimeofday(&t1, nullptr);
    float polygon_load_time = cuspatial::calc_time("h_polygon data loading time ......", t0, t1);

    point_len = cuspatial::read_point_lonlat<T>(point_filename, h_point_x, h_point_y);
    std::cout << "point_len=" << point_len << std::endl;
    CUDF_EXPECTS(point_len > 0, "# of points must be greater than 0");
    CUDF_EXPECTS(h_point_x != nullptr && h_point_y != nullptr,
                 "point h_point_x/h_point_y array can not be nullptr");
    ...
    cudaMemcpy(feature_position.data,
               h_polygon.feature_position,
               h_polygon.num_feature * sizeof(uint32_t),
               cudaMemcpyHostToDevice);
    cudaMemcpy(ring_position.data,
               h_polygon.ring_position,
               h_polygon.num_ring * sizeof(uint32_t),
               cudaMemcpyHostToDevice);
    cudaMemcpy(
      polygon_x.data, h_polygon.x, h_polygon.num_vertex * sizeof(T), cudaMemcpyHostToDevice);
    cudaMemcpy(
      polygon_y.data, h_polygon.y, h_polygon.num_vertex * sizeof(T), cudaMemcpyHostToDevice);
  ...
  {
    gdf_column result_bitmap = cuspatial::point_in_polygon_bitmap(this->point_x,
                                                                  this->point_y,
