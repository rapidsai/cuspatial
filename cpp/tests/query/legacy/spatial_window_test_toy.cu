...
TEST_F(TrajectoryDeriveToy, trajectoryderivetest) {
  ...
  // computed distance and speed are in the units of meters and m/s,
  // respectively
  double point_x[] = {1.0, 2.0, 3.0, 5.0, 7.0, 1.0,
                      2.0, 3.0, 6.0, 0.0, 3.0, 6.0};
  double point_y[] = {0.0, 1.0, 2.0, 3.0, 1.0, 3.0,
                      5.0, 6.0, 5.0, 4.0, 7.0, 4.0};
  ...
  std::cout << "x:" << std::endl;
  std::copy(point_x_vec.begin(), point_x_vec.end(),
            std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
  std::cout << "y:" << std::endl;
  std::copy(point_y_vec.begin(), point_y_vec.end(),
            std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
  ...
  std::cout << "points within query window(" << x1.data.fp64 << ","
            << y1.data.fp64 << "," << x2.data.fp64 << "," << y2.data.fp64 << ")"
            << std::endl;
  thrust::device_ptr<double> out_x_ptr =
      thrust::device_pointer_cast(static_cast<double*>(res_pair.first.data));
  thrust::device_ptr<double> out_y_ptr =
      thrust::device_pointer_cast(static_cast<double*>(res_pair.second.data));
  int num_print = res_pair.first.size;
  std::cout << "x:" << std::endl;
  thrust::copy(out_x_ptr, out_x_ptr + num_print,
               std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
  std::cout << "y:" << std::endl;
  thrust::copy(out_y_ptr, out_y_ptr + num_print,
               std::ostream_iterator<double>(std::cout, " "));
  std::cout << std::endl;
}
