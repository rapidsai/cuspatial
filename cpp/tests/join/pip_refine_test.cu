...
struct PIPRefineTest : public GdfTest 
{
 
};

TEST_F(PIPRefineTest, test1)
{
    ...
    RMM_TRY( RMM_ALLOC( &d_pnt_x,point_len* sizeof(double), stream));
    assert(d_pnt_x!=NULL);
    RMM_TRY( RMM_ALLOC( &d_pnt_y,point_len* sizeof(double), stream));
    assert(d_pnt_y!=NULL);    
    ...
    RMM_TRY( RMM_ALLOC( &d_pnt_id,point_len* sizeof(uint32_t), stream));
    assert(d_pnt_id!=NULL);
    thrust::sequence(thrust::device,d_pnt_id,d_pnt_id+point_len);

    cudf::mutable_column_view pnt_id(cudf::data_type{cudf::INT32},point_len,d_pnt_id);
    cudf::mutable_column_view pnt_x(cudf::data_type{cudf::FLOAT64},point_len,d_pnt_x);
    cudf::mutable_column_view pnt_y(cudf::data_type{cudf::FLOAT64},point_len,d_pnt_y);
    
    std::unique_ptr<cudf::experimental::table> quadtree= cuspatial::quadtree_on_points(pnt_id,pnt_x,pnt_y,x1,y1,x2,y2, scale,num_levels, min_size);
    ...
    std::unique_ptr<cudf::column> poly_id = cudf::make_numeric_column( cudf::data_type{cudf::type_id::INT32}, 
    	num_poly, cudf::mask_state::UNALLOCATED, stream, mr );      
    uint32_t *d_poly_id=cudf::mutable_column_device_view::create(poly_id->mutable_view(), stream)->data<uint32_t>();
    assert(d_poly_id!=NULL);
    thrust::sequence(thrust::device,d_poly_id,d_poly_id+num_poly);
 
    std::unique_ptr<cudf::column> poly_fpos = cudf::make_numeric_column( cudf::data_type{cudf::type_id::INT32}, 
    	num_poly, cudf::mask_state::UNALLOCATED, stream, mr );      
    ...
    HANDLE_CUDA_ERROR( cudaMemcpy( d_poly_fpos, ply_fpos, num_poly * sizeof(uint32_t), cudaMemcpyHostToDevice ) ); 

    std::unique_ptr<cudf::column> poly_rpos = cudf::make_numeric_column( cudf::data_type{cudf::type_id::INT32}, 
    ...
    HANDLE_CUDA_ERROR( cudaMemcpy( d_poly_rpos, ply_rpos, num_ring * sizeof(uint32_t), cudaMemcpyHostToDevice ) ); 

    std::unique_ptr<cudf::column> poly_x = cudf::make_numeric_column( cudf::data_type{cudf::type_id::FLOAT64}, 
    ...
    std::unique_ptr<cudf::column> poly_y = cudf::make_numeric_column( cudf::data_type{cudf::type_id::FLOAT64}, 
    	num_vertex, cudf::mask_state::UNALLOCATED, stream, mr );      
    ...
    std::unique_ptr<cudf::experimental::table> bbox_tbl=cuspatial::polygon_bbox(*poly_fpos,*poly_rpos,*poly_x,*poly_y); 
    ...
    const cudf::table_view pnt_view({pnt_id,pnt_x,pnt_y});
