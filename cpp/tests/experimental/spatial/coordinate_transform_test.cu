...
TYPED_TEST(LonLatToCartesianTest, Empty)
{
  ...
  auto expected     = rmm::device_vector<Loc>{};

  auto xy_output = rmm::device_vector<Loc>{};
...
TYPED_TEST(LonLatToCartesianTest, Single)
{
  ...
  auto h_expected     = std::vector<Loc>({{-0.01126195531216838, -0.21375777777718794}});

  auto point_lonlat = rmm::device_vector<Loc>{h_point_lonlat};
  auto expected     = rmm::device_vector<Loc>{h_expected};

  auto xy_output = rmm::device_vector<Loc>(1);
...
TYPED_TEST(LonLatToCartesianTest, Extremes)
{
  ...
  auto h_expected = std::vector<Loc>({{0.0, 10000.0},
                                      {0.0, -10000.0},
                                      {20000.0, 0.0},
                                      {-20000.0, 0.0},
                                      {-5000.0, 0.0},
  ...
  auto xy_output = rmm::device_vector<Loc>(6, Loc{-1, -1});
...
template <typename T>
struct printer {
  __device__ void operator()(cuspatial::vec_2d<T> const& v) { printf("%f %f\n", v.x, v.y); }
};
...
TYPED_TEST(LonLatToCartesianTest, Multiple)
{
  ...
  auto xy_output = rmm::device_vector<Loc>(4, Loc{-1, -1});
