...
TEST_F(CubicSplineTest, test_interpolate)
{
    ...
    int ids[5] = {0, 0};
    int prefix[2] = {0, 5};
    int point_prefix[5] = {0, 0, 0, 0, 0};
    
    float *d_p_t = NULL;
    float *d_p_x = NULL;
    float *d_p_ids = NULL;
    float *d_p_prefix = NULL;
    float *d_p_point_prefix = NULL;
    RMM_TRY( RMM_ALLOC( &d_p_t, point_len * sizeof(float), 0));
    assert(d_p_t != NULL);    
    ...
    assert(d_p_prefix != NULL);    
    RMM_TRY( RMM_ALLOC( &d_p_point_prefix, 5 * sizeof(int), 0));
    assert(d_p_point_prefix != NULL);    
    ...
    HANDLE_CUDA_ERROR( cudaMemcpy( d_p_point_prefix, point_prefix, 5 * sizeof(int), cudaMemcpyHostToDevice ) );     
    
    cudf::column_view t_column(cudf::data_type{cudf::FLOAT32},point_len,d_p_t);
    cudf::column_view x_column(cudf::data_type{cudf::FLOAT32},point_len,d_p_x);
    cudf::column_view ids_column(cudf::data_type{cudf::INT32},2,d_p_ids);
    cudf::column_view prefix_column(cudf::data_type{cudf::INT32},2,d_p_prefix);
    cudf::column_view point_prefix_column(cudf::data_type{cudf::INT32},5,d_p_point_prefix);
