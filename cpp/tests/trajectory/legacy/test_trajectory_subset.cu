...
void test_subset(std::vector<int32_t> ids_to_keep)
{
  ...
  auto end =
    std::copy_if(sequence.begin(), sequence.end(), expected_sequence.begin(), [&](int32_t i) {
      return std::binary_search(ids_to_keep.begin(), ids_to_keep.end(), id_vector[i]);
    });

  gdf_size_type expected_size = end - expected_sequence.begin();

  wrapper<double> expected_x(
    expected_size, [&](cudf::size_type i) { return static_cast<double>(expected_sequence[i]); });
  wrapper<double> expected_y(
    expected_size, [&](cudf::size_type i) { return static_cast<double>(expected_sequence[i]); });
  wrapper<int32_t> expected_id(expected_size,
                               [&](cudf::size_type i) { return id_vector[expected_sequence[i]]; });
  wrapper<cudf::timestamp> expected_ts(expected_size, [&](cudf::size_type i) {
    return static_cast<cudf::timestamp>(ms_vector[expected_sequence[i]]);
  });

  gdf_size_type num_hit{0};

  EXPECT_NO_THROW(num_hit = cuspatial::subset_trajectory_id(
                    ids, in_x, in_y, in_id, in_ts, out_x, out_y, out_id, out_ts));
...
TEST_F(TrajectorySubsetTest, BadData)
{
  ...
  CUDF_EXPECT_THROW_MESSAGE(
    cuspatial::subset_trajectory_id(
      bad_id, bad_x, bad_y, bad_in_id, bad_timestamp, out_x, out_y, out_id, out_timestamp),
    "Null input data");
