 ...
 */
int get_camera_origin(const char *df_fn,
                      const char *inter_name,
                      location_3d<double> &camera_origin);

/**
 * @brief retrive id, timestamp and location fields of a "raw" trajectory dataset,
 * i.e., a set of coordiantes (lon/lat/alt) with a timestamp and an object (e.g., vehicle)
 * identifier.
 *
 * @param[in]  root_fn: the root of the three files stored in columnar format,
 * with .objectid (uint32_t type),.time (its_timestamp type) and .location(location_3d type)
 ...
 */
size_t read_traj_soa(char *root_fn,
                     int *&objid,
                     its_timestamp *&time,
                     location_3d<double> *&location);
