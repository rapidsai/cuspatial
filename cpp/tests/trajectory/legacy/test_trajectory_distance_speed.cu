...
TEST_F(TrajectoryDistanceSpeed, DistanceAndSpeedThree)
{
  ...
  std::transform(sequence.cbegin(), sequence.cend(), ms_vector.begin(), [](int32_t i) {
    return i * ms_per_hour + i;
  });
  ...
  std::vector<gdf_size_type> trajectory_length{
    2 * column_size / 3, (column_size + 5) / 6, (column_size + 5) / 6};
  std::vector<gdf_size_type> trajectory_offset{
  ...
  EXPECT_NO_THROW(distance_speed = cuspatial::trajectory_distance_and_speed(
                    in_x, in_y, in_ts, traj_len, traj_offset););
  ...
  for (auto i : id) {
    ...
    for (gdf_size_type j = 0; j < length - 1; j++) {
      double d  = static_cast<double>(sequence[begin + j + 1] - sequence[begin + j]);
      double ms = static_cast<double>(ms_vector[begin + j + 1] - ms_vector[begin + j]);
  ...
  cudaMemcpy(gpu_distance.data(),
             distance_speed.first.data,
             distance_speed.first.size * sizeof(double),
             cudaMemcpyDefault);
  std::vector<double> gpu_speed(distance_speed.second.size);
  cudaMemcpy(gpu_speed.data(),
             distance_speed.second.data,
             distance_speed.second.size * sizeof(double),
             cudaMemcpyDefault);
...
TEST_F(TrajectoryDistanceSpeed, BadData)
{
  ...
  CUDF_EXPECT_THROW_MESSAGE(
    cuspatial::trajectory_distance_and_speed(bad_x, bad_y, bad_timestamp, bad_length, bad_offset),
    "Null input data");

  // size mismatch
  bad_x.data = bad_y.data = bad_timestamp.data = bad_length.data = bad_offset.data =
