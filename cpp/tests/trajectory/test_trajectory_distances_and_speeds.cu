...
TYPED_TEST(TrajectoryDistanceSpeedTest,
  ...
  auto test_data =
      cuspatial::test::make_test_trajectories_table<T>(1000, this->mr());
  ...
  // compute expected distance and speed
  std::tie(distance, speed) =
      expected_distance_and_speed<T, Rep>(h_offsets, h_xs, h_ys, h_ts);
...
TYPED_TEST(TrajectoryDistanceSpeedTest, ComputeDistanceAndSpeed3Simple) {
  ...
  auto distance_and_speed =
      cuspatial::experimental::trajectory_distances_and_speeds(3, id, xs, ys,
                                                               ts, this->mr());
  ...
  std::vector<double> speed(
      {1973230.5567480423, 2270853.0666804211, 4242640.6871192846});
  std::vector<double> distance(
      {7892.9222269921693, 6812.5592000412635, 8485.2813742385697});
