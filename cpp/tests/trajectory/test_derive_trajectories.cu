...
TEST_F(DeriveTrajectoriesTest, DerivesThreeTrajectories) {
  ...
  std::transform(map.cbegin(), map.cend(), ids.begin(), [](int32_t i) {
  ...
  auto result =
      cuspatial::experimental::derive_trajectories(id, x, y, ts, this->mr());

  auto expected_ids = cudf::test::fixed_width_column_wrapper<int32_t>{0, 1, 2};
  auto expected_lengths = cudf::test::fixed_width_column_wrapper<int32_t>{
      2 * size / 3, (size + 5) / 6, (size + 5) / 6};
  auto expected_offsets = cudf::test::fixed_width_column_wrapper<int32_t>{
      2 * size / 3, 5 * size / 6, size};

  cudf::test::expect_columns_equal(result->get_column(0), expected_ids);
  cudf::test::expect_columns_equal(result->get_column(1), expected_lengths);
  cudf::test::expect_columns_equal(result->get_column(2), expected_offsets);
}
