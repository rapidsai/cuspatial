 ...
 * scale works with x1 and x2 to convert x/y coodiantes into a Morton code in 2D
space

 *@ param[in] num_level: largest depth of quadtree nodes
 * the value should be less than 16 as uint32_t is used for Morton code
representation
 * the actual number of levels may be less than num_level
 * when #of points are small and/or min_size (next) is large

 *@ param[in] min_size: the minimum number of points for a non-leaf quadtree
node
 ...
 * @return experimental::table with five columns for a complete quadtree:
...
std::unique_ptr<cudf::experimental::table> quadtree_on_points(
    ...
    double y1, double x2, double y2, double scale, int32_t num_level,
    int32_t min_size);
