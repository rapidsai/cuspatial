...
public:
    segment_method_view(
        ParentRange range, IndexRange non_empty_geometry_prefix_sum,
        index_t num_segments, bool contains_empty_ring
    ) : _range(range),
    ...
    auto non_empty_linestring_count_begin() {
        ...
        auto paired_begin = thrust::make_zip_iterator(
            begin, thrust::next(begin)
        );
        return thrust::make_transform_iterator(
            paired_begin, offset_pair_to_count_functor{}
        );
    }
    ...
    auto segment_count_begin() {
        ...
        auto n_point_linestring_pair_it = thrust::make_zip_iterator(
            num_points_begin, this->non_empty_linestring_count_begin()
        );

        return thrust::make_transform_iterator(
            n_point_linestring_pair_it, point_count_to_segment_count_functor{}
        );
    }

    auto segment_count_end()   {
        return thrust::next(this->segment_count_begin(), _num_segments);
    }
...
template<typename ParentRange, typename IndexRange>
segment_method_view(
    ParentRange, IndexRange, typename IndexRange::value_type, bool
