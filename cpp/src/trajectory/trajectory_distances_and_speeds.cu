...
template <typename T>
auto duplicate_first_element_iterator(cudf::column_view const& col,
                                      cudaStream_t stream) {
...
struct dispatch_timestamp {
  ...
  operator()(cudf::column_view const& object_id, cudf::column_view const& x,
    ...
    auto timestamp_point_and_id = thrust::make_zip_iterator(thrust::make_tuple(
        duplicate_first_element_iterator<Rep>(timestamp, stream),  //
    ...
    auto duration_and_distance_1 = thrust::make_zip_iterator(thrust::make_tuple(
        durations.begin(),  //
        distances.begin(),  //
    ...
    auto duration_and_distance_2 = thrust::make_zip_iterator(
        thrust::make_tuple(durations.begin(), distances.begin(),
