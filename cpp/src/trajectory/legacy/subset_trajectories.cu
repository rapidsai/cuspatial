...
struct is_true {
  __device__ bool operator()(const bool t) { return (t); }
};
...
struct subset_functor {
                           ...
                           gdf_column& out_timestamp)
  {
    ...
    if (num_id > 0 && id.data != nullptr && num_rec > 0) {
      ...
      thrust::binary_search(exec_policy->on(stream),
                            temp_id.cbegin(),
                            temp_id.cend(),
                            in_id_ptr,
                            in_id_ptr + num_rec,
      ...
      num_hit = thrust::count(exec_policy->on(stream), hit_vec.begin(), hit_vec.end(), true);
      ...
      if (num_hit > 0) {
        ...
        auto in_itr = thrust::make_zip_iterator(
          thrust::make_tuple(static_cast<T*>(in_x.data),
                             static_cast<T*>(in_y.data),
                             static_cast<int32_t*>(in_id.data),
                             static_cast<cudf::timestamp*>(in_timestamp.data)));
        auto out_itr = thrust::make_zip_iterator(
          thrust::make_tuple(static_cast<T*>(out_x.data),
                             static_cast<T*>(out_y.data),
                             static_cast<int32_t*>(out_id.data),
                             static_cast<cudf::timestamp*>(out_timestamp.data)));

        auto end = thrust::copy_if(
          exec_policy->on(stream), in_itr, in_itr + num_rec, hit_vec.begin(), out_itr, is_true());
        gdf_size_type num_keep = end - out_itr;

        CUSPATIAL_EXPECTS(num_hit == num_keep, "count_if and copy_if result mismatch");
      }
    }
...
gdf_size_type subset_trajectory_id(const gdf_column& id,
                                   ...
                                   gdf_column& out_timestamp)
{
  CUSPATIAL_EXPECTS(in_x.data != nullptr && in_x.data != nullptr && in_id.data != nullptr &&
                      in_timestamp.data != nullptr,
                    "Null input data");
  CUSPATIAL_EXPECTS(
    in_x.size == in_y.size && in_x.size == in_id.size && in_x.size == in_timestamp.size,
    "Data size mismatch");
  CUSPATIAL_EXPECTS(in_id.dtype == GDF_INT32, "Invalid trajectory ID datatype");
  CUSPATIAL_EXPECTS(id.dtype == in_id.dtype, "Trajectory ID datatype mismatch");
  CUSPATIAL_EXPECTS(in_timestamp.dtype == GDF_TIMESTAMP, "Invalid timestamp datatype");
  CUSPATIAL_EXPECTS(in_x.null_count == 0 && in_y.null_count == 0 && in_id.null_count == 0 &&
                      in_timestamp.null_count == 0,
  ...
  return cudf::type_dispatcher(in_x.dtype,
                               subset_functor(),
                               id,
                               in_x,
                               in_y,
