...
struct derive_trajectories_functor {
  ...
  gdf_size_type operator()(const gdf_column& x,
                           const gdf_column& y,
                           ...
                           gdf_column& offset)
  {
    ...
    cudf::timestamp* time_ptr = static_cast<cudf::timestamp*>(timestamp.data);

    gdf_size_type num_rec = object_id.size;
    thrust::stable_sort_by_key(exec_policy->on(stream),
                               time_ptr,
                               time_ptr + num_rec,
    ...
    auto end               = thrust::reduce_by_key(exec_policy->on(stream),
                                     id_ptr,
                                     id_ptr + num_rec,
    ...
    thrust::inclusive_scan(exec_policy->on(stream), traj_count, traj_count + num_traj, traj_pos);
...
gdf_size_type derive_trajectories(const gdf_column& x,
                                  ...
                                  gdf_column& offset)
{
  CUSPATIAL_EXPECTS(x.data != nullptr && y.data != nullptr && object_id.data != nullptr &&
                      timestamp.data != nullptr,
                    "Null input data");
  CUSPATIAL_EXPECTS(x.size == y.size && x.size == object_id.size && x.size == timestamp.size,
                    "Data size mismatch");
  CUSPATIAL_EXPECTS(object_id.dtype == GDF_INT32, "Invalid trajectory ID datatype");
  CUSPATIAL_EXPECTS(timestamp.dtype == GDF_TIMESTAMP, "Invalid timestamp datatype");
  CUSPATIAL_EXPECTS(x.null_count == 0 && y.null_count == 0 && object_id.null_count == 0 &&
                      timestamp.null_count == 0,
                    "NULL support unimplemented");

  gdf_size_type num_trajectories = cudf::type_dispatcher(x.dtype,
