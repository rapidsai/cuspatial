...
struct distspeed_functor {
                                               ...
                                               const gdf_column& offset)
  {
    ...
    gdf_column_view_augmented(&dist,
                              temp,
                              nullptr,
                              length.size,
                              x.dtype,
    ...
    gdf_column speed{};
    RMM_TRY(RMM_ALLOC(&temp, length.size * sizeof(T), 0));
    gdf_column_view_augmented(&speed,
                              temp,
                              nullptr,
                              length.size,
                              x.dtype,
    ...
    CUDA_TRY(cudaOccupancyMaxPotentialBlockSize(&min_grid_size, &block_size, distspeed_kernel<T>));
    cudf::util::cuda::grid_config_1d grid{x.size, block_size, 1};
    distspeed_kernel<T>
      <<<grid.num_blocks, block_size>>>(length.size,
                                        ...
                                        static_cast<T*>(dist.data),
                                        static_cast<T*>(speed.data));
...
std::pair<gdf_column, gdf_column> trajectory_distance_and_speed(const gdf_column& x,
                                                                ...
                                                                const gdf_column& offset)
{
  CUSPATIAL_EXPECTS(x.data != nullptr && y.data != nullptr && timestamp.data != nullptr &&
                      length.data != nullptr && offset.data != nullptr,
                    "Null input data");
  CUSPATIAL_EXPECTS(x.size == y.size && x.size == timestamp.size && length.size == offset.size,
                    "Data size mismatch");
  CUSPATIAL_EXPECTS(timestamp.dtype == GDF_TIMESTAMP, "Invalid timestamp datatype");
  CUSPATIAL_EXPECTS(length.dtype == GDF_INT32, "Invalid trajectory length datatype");
  CUSPATIAL_EXPECTS(offset.dtype == GDF_INT32, "Invalid trajectory offset datatype");
  CUSPATIAL_EXPECTS(x.null_count == 0 && y.null_count == 0 && timestamp.null_count == 0 &&
                      length.null_count == 0 && offset.null_count == 0,
                    "NULL support unimplemented");
  CUSPATIAL_EXPECTS(x.size >= offset.size, "Insufficient trajectory data");

  std::pair<gdf_column, gdf_column> res_pair =
    cudf::type_dispatcher(x.dtype, distspeed_functor(), x, y, timestamp, length, offset);
