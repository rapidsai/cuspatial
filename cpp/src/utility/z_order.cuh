...
/*  Algorithm 1  */
__device__ inline uint32_t dilate_2(uint16_t x) {
  return dilate_tab2[0xFF & x] | (dilate_tab2[(0xFFFF & x) >> 8] << 16);
}

__device__ inline uint32_t z_order(uint16_t x, uint16_t y) {
  return (dilate_2(y) << 1) | dilate_2(x);
}

__constant__ const unsigned char undilate_tab2[256] = {
    };

/*  Algorithm 2  */
__device__ inline uint16_t undilate_2(uint32_t x) {
  return undilate_tab2[0xFF & ((x >> 7) | x)] |
         (undilate_tab2[0xFF & (((x >> 7) | x) >> 16)] << 8);
}

__device__ inline uint16_t z_order_x(uint32_t index) {
  return undilate_2(index & 0x55555555);
}

__device__ inline uint16_t z_order_y(uint32_t index) {
