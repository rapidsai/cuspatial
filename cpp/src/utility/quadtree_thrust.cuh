...
struct get_parent {
  ...
  __device__ uint32_t operator()(uint32_t child) { return (child >> lev); }
};
...
struct gen_quad_bbox {
  ...
  __device__ SBBox<T> operator()(uint32_t p) const {
    ...
    // printf("gen_quad_bbox: %5d %5d %5d %10.5f %10.5f %10.5f %10.5f
...
struct qt_get_fpos {
  ...
  __device__ uint32_t operator()(uint32_t idx) { return d_p_qtfpos[idx]; }
};
