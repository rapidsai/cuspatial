...
template <typename T>
struct polygons {
  uint32_t num_group, num_feature, num_ring,
    num_vertex;                         // numbers of groups, features,rings and vertices
...
void read_polygon_soa(const char *poly_filename, struct polygons<T> *ply)
{
  ...
  *ply = polygons<T>{};
  ...
  CUDF_EXPECTS(
    ply->num_group > 0 && ply->num_feature > 0 && ply->num_ring > 0 && ply->num_vertex > 0,
    "numbers of groups/features/rings/vertices must be positive");
  CUDF_EXPECTS(ply->num_group <= ply->num_feature && ply->num_feature <= ply->num_ring &&
                 ply->num_ring <= ply->num_vertex,
  ...
  size_t len = (4 + ply->num_group + ply->num_feature + ply->num_ring) * sizeof(int) +
               2 * ply->num_vertex * sizeof(T);
  ...
  CUDF_EXPECTS(
    ply->group_length != nullptr && ply->feature_length != nullptr && ply->ring_length != nullptr,
    "expecting p_{g,f,r}_len are non-zeron");
  ply->x = new T[ply->num_vertex];
  ply->y = new T[ply->num_vertex];
  CUDF_EXPECTS(ply->x != nullptr && ply->y != nullptr,
               "expecting polygon x/y arrays are not nullptr");
  ...
  CUDF_EXPECTS(r_g == ply->num_group && r_f == ply->num_feature && r_r == ply->num_ring &&
                 r_x == ply->num_vertex && r_y == ply->num_vertex,
