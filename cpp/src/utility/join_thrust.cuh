...
{    
    ...
    const uint32_t size;
    get_num_units(const uint32_t *_qt_len,const uint32_t _size):
    	qt_len(_qt_len),size(_size){ }
    	
    __device__ 
    uint32_t operator()(uint32_t qid)
    {
...
{    
    ...
    gen_offset_length(const uint32_t _N,const uint32_t *_qt_length):
    	N(_N),qt_length(_qt_length){ }
    	
    __device__ 
    thrust::tuple<uint32_t,uint32_t> operator()(thrust::tuple<uint32_t,uint32_t> v)
    {
...
{    
    __device__ 
    bool operator()(thrust::tuple<uint32_t,uint32_t,uint32_t,uint32_t,uint32_t> v)
    {
