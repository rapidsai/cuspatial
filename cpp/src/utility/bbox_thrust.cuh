...
struct bbox_reduction {
  __device__ SBBox<T> operator()(const SBBox<T> &a, const SBBox<T> &b) {
    ...
    thrust::tuple<double, double> ll =
        thrust::make_tuple(min(fx1, fx2), min(fy1, fy2));
    thrust::tuple<double, double> ur =
        thrust::make_tuple(max(tx1, tx2), max(ty1, ty2));

    /*printf("[(%7.5f %7.5f %7.5f %7.5f),(%7.5f,%7.5f %7.5f
    %7.5f)]==>((%7.5f,%7.5f %7.5f %7.5f)\n", fx1,fy1,tx1,ty1,fx2,fy2,tx2,ty2,
    min(fx1,fx2),min(fy1,fy2),max(tx1,tx2),max(ty1,ty2));*/
...
struct bbox_transformation {
  __device__ SBBox<T> operator()(const thrust::tuple<T, T> &t) {
    return SBBox<T>(t, t);
  }
};
...
template <typename T>
struct tuple2bbox {
  __device__ SBBox<T> operator()(thrust::tuple<T, T, T, T> v) {
    T x1 = thrust::get<0>(v);
    T y1 = thrust::get<1>(v);
    T x2 = thrust::get<2>(v);
    T y2 = thrust::get<3>(v);
...
struct pairwise_test_intersection {
  ...
  pairwise_test_intersection(uint32_t _M, uint32_t _num_node,
                             const SBBox<double> &_aoi_bbox, double _scale,
                             const uint32_t *_d_p_key, const uint8_t *_d_p_lev,
                             const bool *_d_p_sign, const SBBox<T> *_ply_bbox)
      : M(_M),
        num_node(_num_node),
        aoi_bbox(_aoi_bbox),
        d_p_key(_d_p_key),
        d_p_lev(_d_p_lev),
        d_p_qtsign(_d_p_sign),
        scale(_scale),
        ply_bbox(_ply_bbox) {
    // printf("pairwise_test_intersection: num_node=%d\n",num_node);
  }

  __device__ thrust::tuple<uint8_t, uint8_t, uint32_t, uint32_t> operator()(
      uint32_t p) const {
    ...
    // printf("init: tid=%d qx1=%10.5f qy1=%10.5f qx2=%10.5f qy2=%10.5f
    // px1=%10.5f py1=%10.5f px2=%10.5f
    // py2=%10.5f\n",tid,qx1,qy1,qx2,qy2,px1,py1,px2,py2); printf("init: tid=%d
    // lev=%d sign=%d type=%d poly_idx=%d quad_id=%d\n",tid,
    // lev,d_p_qtsign[quad_idx],type,poly_idx,quad_idx);
...
struct twolist_test_intersection {
  ...
  twolist_test_intersection(uint32_t _M, const SBBox<double> &_aoi_bbox,
                            double _scale, const uint32_t *_d_p_key,
                            const uint8_t *_d_p_lev, const bool *_d_p_sign,
                            const SBBox<T> *_ply_bbox)
      : M(_M),
        aoi_bbox(_aoi_bbox),
        d_p_key(_d_p_key),
        d_p_lev(_d_p_lev),
  ...
  __device__ thrust::tuple<uint8_t, uint8_t, uint32_t, uint32_t> operator()(
      thrust::tuple<uint32_t, uint32_t> v) const {
    ...
    uint8_t type = not_intersect ? 2 : (d_p_qtsign[quad_idx] ? 1 : 0);
    // uint32_t tid = threadIdx.x + blockDim.x*blockIdx.x;
    // printf("rest: tid=%d qx1=%10.5f qy1=%10.5f qx2=%10.5f qy2=%10.5f
    // px1=%10.5f py1=%10.5f px2=%10.5f
    // py2=%10.5f\n",tid,qx1,qy1,qx2,qy2,px1,py1,px2,py2); printf("rest: tid=%d
    // lev=%d sign=%d type=%d poly_idx=%d quad_id=%d\n",tid,
    // lev,d_p_qtsign[quad_idx],type,poly_idx,quad_idx);
    return thrust::make_tuple(lev, type, poly_idx, quad_idx);
  }
