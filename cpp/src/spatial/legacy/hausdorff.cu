...
template <typename T>
__global__ void kernel_Hausdorff_Full(int num_traj, T* xx, T* yy, uint32_t* pos, T* results)
{
  ...
  if (bidx < num_traj * num_traj) {
    ...
    int max_threads = 0;
    {
      ...
      if (threadIdx.x < max_threads) {
        ...
        for (int i = start_right; i < stop_right; i++) {
          ...
          T other_yy = yy[i];
          T new_dist =
            (my_xx - other_xx) * (my_xx - other_xx) + (my_yy - other_yy) * (my_yy - other_yy);
    ...
    if (threadIdx.x < max_threads) sdata[threadIdx.x] = dist;
    __syncthreads();
    // reduction
    ...
    __syncthreads();
    if (threadIdx.x == 0) results[bidx] = (sdata[0] >= 0) ? sqrt(sdata[0]) : 1e10;
  }
}
...
struct Hausdorff_functor {
  ...
  gdf_column operator()(const gdf_column& x, const gdf_column& y, const gdf_column& vertex_counts)
  {
    ...
    thrust::inclusive_scan(
      exec_policy->on(stream), vertex_counts_ptr, vertex_counts_ptr + num_set, vertex_positions);
    ...
    kernel_Hausdorff_Full<T><<<grid, block>>>(
      num_set, static_cast<T*>(x.data), static_cast<T*>(y.data), vertex_positions, temp_matrix);

    CUDA_TRY(cudaDeviceSynchronize());
    RMM_TRY(RMM_FREE(vertex_positions, stream));

    gdf_column_view_augmented(&d_matrix,
                              temp_matrix,
                              nullptr,
                              block_sz,
                              x.dtype,
...

{
  ...
  // future versions might allow x/y/vertex_counts have null_count>0, which might be useful for
  // taking query results as inputs
