...
struct pariwise_linestring_distance_functor {
    ...
    rmm::mr::device_memory_resource* mr)
  {
    ...
    auto min_distances =
      cudf::make_numeric_column(cudf::data_type{cudf::type_to_id<T>()}, num_string_pairs, cudf::mask_state::UNALLOCATED, stream, mr);

    auto linestring1_coords_it = make_coord_2d_iterator<cart_2d<T>>(linestring1_points_x.begin<T>(), linestring1_points_y.begin<T>());
    auto linestring2_coords_it = make_coord_2d_iterator<cart_2d<T>>(linestring2_points_x.begin<T>(), linestring2_points_y.begin<T>());
