...
struct bounding_box_processor {
                                                        ...
                                                        cudaStream_t stream)
  {
    ...
    // std::cout<<"bounding_box_processor:
    // num_poly="<<num_poly<<",num_ring="<<num_ring<<",num_vertex="<<num_vertex<<std::endl;
    ...
    rmm::device_buffer *db_first_ring_pos =
    ...
    rmm::device_buffer *db_temp_ring_pos =
      new rmm::device_buffer(num_poly * sizeof(uint32_t), stream, mr);
    CUDF_EXPECTS(db_temp_ring_pos != nullptr,
    ...
    CUDA_TRY(cudaMemset(d_first_ring_pos, 0, num_poly * sizeof(uint32_t)));
    ...
    if (0) {
      ...
      thrust::copy(
        d_fpos_ptr, d_fpos_ptr + num_poly, std::ostream_iterator<uint32_t>(std::cout, " "));
      std::cout << std::endl;

      printf("vertex pos prefix sum\n");
      thrust::device_ptr<const uint32_t> d_rpos_ptr = thrust::device_pointer_cast(d_ply_rpos);
      thrust::copy(
        d_rpos_ptr, d_rpos_ptr + num_ring, std::ostream_iterator<uint32_t>(std::cout, " "));
      std::cout << std::endl;
    }
    ...
    thrust::gather(exec_policy->on(stream),
                   d_temp_ring_pos,
                   d_temp_ring_pos + num_poly,
                   d_ply_rpos,
                   d_first_ring_pos);

    delete db_temp_ring_pos;
    db_temp_ring_pos = nullptr;
    ...
    if (0) {
      ...
      thrust::copy(d_num_points_ptr,
                   d_num_points_ptr + num_poly,
                   std::ostream_iterator<uint32_t>(std::cout, " "));
      std::cout << std::endl;
    }

    thrust::adjacent_difference(
      exec_policy->on(stream), d_first_ring_pos, d_first_ring_pos + num_poly, d_first_ring_pos);
    ...
    if (0) {
      ...
      thrust::copy(d_num_rings_ptr,
                   d_num_rings_ptr + num_poly,
                   std::ostream_iterator<uint32_t>(std::cout, " "));
      std::cout << std::endl;
    }
    ...
    delete db_first_ring_pos;
    db_first_ring_pos = nullptr;
    ...
    thrust::transform(
      exec_policy->on(stream), d_p_bbox, d_p_bbox + num_bbox, out_bbox_iter, bbox2tuple<T>(0));
