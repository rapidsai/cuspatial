...
*/
    ...
    CUDF_EXPECTS(d_pnt_qtsign!=NULL," error allocating storage memory for sign column");
...
*/
   ...
   CUDF_EXPECTS(d_pnt_qtlength!=NULL," error allocating memory storage for length column");
   ...
   CUDF_EXPECTS(d_pnt_qtfpos!=NULL," error allocating memory storage for fpos column");
