...
template <typename T>
inline rmm::device_vector<uint32_t> compute_point_keys(cudf::mutable_column_view &x,
                                                       ...
                                                       cudaStream_t stream)
{
  ...
  thrust::transform(
    policy->on(stream), points, points + x.size(), keys.begin(), [=] __device__(auto const &point) {
      T x, y;
...
template <typename InputIterator1,
          ...
          typename BinaryOp>
inline cudf::size_type construct_quadrant(InputIterator1 keys_begin,
...
{
  auto policy = rmm::exec_policy(stream);
  auto result = thrust::reduce_by_key(policy->on(stream),
                                      keys_begin,
                                      keys_end,
                                      vals_in,
                                      keys_out,
                    ...
                    cudaStream_t stream)
{
  ...
  std::vector<cudf::size_type> begin_pos(num_levels);
  std::vector<cudf::size_type> end_pos(num_levels);
  ...
  for (cudf::size_type level = num_levels - 1; level >= 0; --level) {
    auto num_full_quads = construct_quadrant(parent_keys + begin,
                                             parent_keys + end,
                                             child_values + begin,
                                             keys_begin + end,
                                             child_nodes + end,
                    ...
                    cudaStream_t stream)
{
  ...
  return std::make_tuple(quad_keys, quad_point_count, quad_child_count, quad_levels);
}
  ...
  cudaStream_t stream)
{
  ...
  auto parent_pos = [&]() {
    ...
    thrust::scatter(policy->on(stream),
  ...
  thrust::inclusive_scan(policy->on(stream),
                         parent_pos.begin(),
  ...
  cudaStream_t stream)
{
  ...
  auto num_invalid_parent_nodes =
    thrust::count_if(policy->on(stream),
                     parent_point_counts,
  ...
  auto last_valid =
    thrust::remove_if(policy->on(stream),
                      tree,
  ...
  cudaStream_t stream)
{
  ...
  thrust::transform(policy->on(stream),
  ...
  thrust::replace_if(policy->on(stream),
                     quad_point_count.begin(),
                     quad_point_count.begin() + num_parent_nodes,
                     indicator->view().begin<bool>(),
  ...
  if (num_valid_nodes > num_parent_nodes) {
    ...
    thrust::fill(policy->on(stream),
                 ...
                 indicator->mutable_view().end<bool>(),
                 0);
  }
                                                           ...
                                                           cudaStream_t stream)
{
  ...
  auto result = thrust::copy_if(policy->on(stream),
                                thrust::make_counting_iterator(0),
  ...
  cudaStream_t stream)
{
  ...
  thrust::transform(policy->on(stream),
                    ...
                    [M = num_levels] __device__(auto const &val) {
                      bool is_node{false};
                      uint32_t key{}, level{};
  ...
  cudaStream_t stream)
{
  ...
  std::tie(initial_sort_indices, quad_point_count_tmp) = [&]() {
    auto flattened_keys =
      flatten_point_keys(quad_keys, quad_level, indicator, num_valid_nodes, num_levels, stream);

    rmm::device_vector<uint32_t> initial_sort_indices(num_valid_nodes);
    thrust::sequence(policy->on(stream), initial_sort_indices.begin(), initial_sort_indices.end());
  ...
  thrust::stable_sort_by_key(
    policy->on(stream), initial_sort_indices.begin(), initial_sort_indices.end(), count_and_f_pos);
...
inline std::unique_ptr<cudf::column> copy_if_else(rmm::device_vector<TypeIn> const &lhs,
                                                  rmm::device_vector<TypeIn> const &rhs,
  ...
  cudaStream_t stream)
{
  ...
  auto num_child_nodes = thrust::reduce(
    policy->on(stream), quad_child_count.begin(), quad_child_count.begin() + num_parent_nodes);
  ...
  auto indicator = construct_non_leaf_indicator(
  ...
  auto f_pos = [&]() {
    ...
    auto quad_point_pos = compute_flattened_first_point_positions(
      quad_keys, quad_levels, quad_point_count, *indicator, num_valid_nodes, num_levels, stream);
    ...
    return copy_if_else<int32_t>(
      quad_child_pos, quad_point_pos, *indicator, num_valid_nodes, mr, stream);
  }();

  // Construct the lengths output column
  auto lengths = copy_if_else<int32_t>(
    quad_child_count, quad_point_count, *indicator, num_valid_nodes, mr, stream);
  ...
  thrust::copy(
    policy->on(stream), quad_keys.begin(), quad_keys.end(), keys->mutable_view().begin<uint32_t>());
  ...
  cudaStream_t stream)
{
  ...
  std::tie(quad_keys,
    ...
    auto point_keys = compute_point_keys<T>(x, y, x1, y1, x2, y2, scale, num_levels, stream);
    ...
    // the same quadrant.
    auto num_top_quads = construct_quadrant(point_keys.begin(),
                                            point_keys.end(),
    ...
    // Repurpose the point_keys vector now that we have the top-level quads
    auto quad_child_count = std::move(point_keys);
    ...
    //                                                           (root)  |
    //
    std::tie(num_parent_nodes, quad_tree_size, begin_pos, end_pos) =
      construct_quadrants(num_levels,
                          num_top_quads,
                          quad_keys.begin(),
                          quad_point_count.begin(),
                          quad_child_count.begin(),
    ...
    if (num_parent_nodes <= 0) {
      return std::make_tuple(quad_keys,
                             quad_point_count,
                             quad_child_count,
                             quad_levels,
                             num_top_quads,
    ...
    std::tie(
      quad_keys_reversed, quad_point_count_reversed, quad_child_count_reversed, quad_levels) =
      reverse_tree_levels(
        quad_keys, quad_point_count, quad_child_count, begin_pos, end_pos, num_levels, stream);

    return std::make_tuple(quad_keys_reversed,
                           quad_point_count_reversed,
                           quad_child_count_reversed,
                           quad_levels,
                           num_top_quads,
                           num_parent_nodes,
  ...
  // Optimization: return early if the top level nodes are all leaves
  if (num_parent_nodes <= 0) {
    return make_leaf_tree(quad_keys, quad_point_count, num_top_quads, mr, stream);
  }

  return make_quad_tree(quad_keys,
                        quad_point_count,
                        quad_child_count,
                        quad_levels,
                        num_parent_nodes,
...
struct dispatch_construct_quadtree {
  ...
  inline std::unique_ptr<cudf::experimental::table> operator()(cudf::mutable_column_view &x,
                                                               cudf::mutable_column_view &y,
                                                               double const x1,
                                                               double const y1,
                                                               double const x2,
...
std::unique_ptr<cudf::experimental::table> quadtree_on_points(cudf::mutable_column_view x,
                                                              cudf::mutable_column_view y,
                                                              double const x1,
                                                              double const y1,
                                                              double const x2,
                                                              ...
                                                              rmm::mr::device_memory_resource *mr)
{
  ...
  return detail::quadtree_on_points(
    x, y, x1, y1, x2, y2, scale, num_levels, min_size, mr, cudaStream_t{0});
}
