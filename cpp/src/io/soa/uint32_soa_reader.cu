...
gdf_column read_uint32_soa(const char *filename)
{
  ...
  if (data == nullptr) return values;

  uint32_t *temp_val{nullptr};
  RMM_TRY(RMM_ALLOC(&temp_val, num_l * sizeof(uint32_t), 0));
  cudaStream_t stream{0};
  CUDA_TRY(
    cudaMemcpyAsync(temp_val, data, num_l * sizeof(uint32_t), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(
    &values, temp_val, nullptr, num_l, GDF_INT32, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "id");
  return values;
}  // read_uint32_soa
