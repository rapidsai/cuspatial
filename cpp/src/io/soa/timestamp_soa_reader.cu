...
gdf_column read_timestamp_soa(const char* filename)
{
  ...
  if (timestamp == nullptr) return ts;

  its_timestamp* temp_ts{nullptr};
  RMM_TRY(RMM_ALLOC(&temp_ts, num_t * sizeof(its_timestamp), 0));
  cudaStream_t stream{0};
  CUDA_TRY(cudaMemcpyAsync(
    temp_ts, timestamp, num_t * sizeof(its_timestamp), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(
    &ts, temp_ts, nullptr, num_t, GDF_INT64, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "timestamp");
  delete[] timestamp;
