...
std::pair<gdf_column, gdf_column> read_lonlat_points_soa(const char* filename)
{
  ...
  CUDA_TRY(
    cudaMemcpyAsync(temp_lon, p_lon, num_p * sizeof(double), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(&pnt_lon,
                            temp_lon,
                            nullptr,
                            num_p,
                            GDF_FLOAT64,
  ...
  CUDA_TRY(
    cudaMemcpyAsync(temp_lat, p_lat, num_p * sizeof(double), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(&pnt_lat,
                            temp_lat,
                            nullptr,
                            num_p,
                            GDF_FLOAT64,
...
std::pair<gdf_column, gdf_column> read_xy_points_soa(const char* filename)
{
  ...
  cudaStream_t stream{0};
  CUDA_TRY(cudaMemcpyAsync(temp_x, p_x, num_p * sizeof(double), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(
    &pnt_x, temp_x, nullptr, num_p, GDF_FLOAT64, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "x");
  delete[] p_x;

  double* temp_y{nullptr};
  RMM_TRY(RMM_ALLOC(&temp_y, num_p * sizeof(double), 0));
  CUDA_TRY(cudaMemcpyAsync(temp_y, p_y, num_p * sizeof(double), cudaMemcpyHostToDevice, stream));
  gdf_column_view_augmented(
    &pnt_y, temp_y, nullptr, num_p, GDF_FLOAT64, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "y");
  delete[] p_y;
