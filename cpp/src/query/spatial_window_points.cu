...
struct sw_point_functor {
                                               ...
                                               const gdf_column& y)
  {
    ...
    CUSPATIAL_EXPECTS(q_left < q_right, "left must be less than right in a spatial window query");
    CUSPATIAL_EXPECTS(q_bottom < q_top, "bottom must be less than top in a spatial window query");

    auto in_it = thrust::make_zip_iterator(
      thrust::make_tuple(static_cast<T*>(x.data), static_cast<T*>(y.data)));

    int num_hits = thrust::count_if(exec_policy->on(stream),
                                    in_it,
                                    in_it + x.size,
    ...
    thrust::copy_if(exec_policy->on(stream),
                    in_it,
                    in_it + x.size,
                    out_it,
                    spatial_window_functor_xy<T>(q_left, q_bottom, q_right, q_top));
    ...
    gdf_column_view_augmented(
      &out_x, temp_x, nullptr, num_hits, x.dtype, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "x");
    gdf_column_view_augmented(
      &out_y, temp_y, nullptr, num_hits, y.dtype, 0, gdf_dtype_extra_info{TIME_UNIT_NONE}, "y");
                                                        ...
                                                        const gdf_column& y)
{
  ...
  std::pair<gdf_column, gdf_column> res =
    cudf::type_dispatcher(x.dtype, sw_point_functor(), left, bottom, right, top, x, y);
