...
template <typename T>
std::vector<std::unique_ptr<cudf::column>> dowork(uint32_t num_pair,
                                                  const uint32_t *d_poly_idx,
                                                  const uint32_t *d_quad_idx,
                                                  uint32_t num_node,
                                                  ...
                                                  cudaStream_t stream)
{
  ...
  thrust::sort_by_key(
    exec_policy->on(stream), d_temp_quad_idx, d_temp_quad_idx + num_pair, d_temp_poly_idx);
  if (1) {
    std::cout << "temp_quad_idx" << std::endl;
    thrust::device_ptr<uint32_t> temp_quad_idx_ptr = thrust::device_pointer_cast(d_temp_quad_idx);
    thrust::copy(temp_quad_idx_ptr,
                 temp_quad_idx_ptr + num_pair,
    ...
    thrust::copy(temp_poly_idx_ptr,
                 temp_poly_idx_ptr + num_pair,
                 std::ostream_iterator<uint32_t>(std::cout, " "));
    std::cout << std::endl;
  }
  ...
  thrust::inclusive_scan(
    exec_policy->on(stream), d_temp_pidx_fpos, d_temp_pidx_fpos + num_quads, d_temp_pidx_fpos);

  if (1) {
    std::cout << "temp_quad_idx" << std::endl;
    thrust::device_ptr<uint32_t> temp_quad_idx_ptr = thrust::device_pointer_cast(d_temp_quad_idx);
    thrust::copy(temp_quad_idx_ptr,
                 temp_quad_idx_ptr + num_quads,
                 std::ostream_iterator<uint32_t>(std::cout, " "));
    std::cout << std::endl;
    ...
    thrust::copy(temp_pid_fpos_ptr,
                 temp_pid_fpos_ptr + num_quads,
                 std::ostream_iterator<uint32_t>(std::cout, " "));
    std::cout << std::endl;
  }
...
struct nn_distance_processor {
                                          ...
                                          cudaStream_t stream)
  {
    ...
    std::cout << "num_pair=" << num_pair << " num_node=" << num_node << " num_poly=" << num_poly
              << " num_pnt" << num_pnt << std::endl;

    std::vector<std::unique_ptr<cudf::column>> res_cols = dowork(num_pair,
                                                                 d_pq_poly_id,
                                                                 d_pq_quad_id,
                                                                 num_node,
                                                                 d_qt_key,
    ...
    std::unique_ptr<cudf::table> destination_table =
      std::make_unique<cudf::table>(std::move(res_cols));
...
namespace cuspatial {
std::unique_ptr<cudf::table> p2p_nn_refine(cudf::table_view const &pq_pair,
                                           cudf::table_view const &quadtree,
                                           cudf::table_view const &pnt,
                                           cudf::column_view const &poly_spos,
                                           cudf::column_view const &poly_x,
