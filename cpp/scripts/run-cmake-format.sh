...
status=0
if [ -z ${CUSPATIAL_ROOT:+PLACEHOLDER} ]; then
    CUSPATIAL_BUILD_DIR=$(git rev-parse --show-toplevel 2>&1)/cpp/build
    status=$?
else
    CUSPATIAL_BUILD_DIR=${CUSPATIAL_ROOT}
fi

if ! [ ${status} -eq 0 ]; then
    if [[ ${CUSPATIAL_BUILD_DIR} == *"not a git repository"* ]]; then
        echo "This script must be run inside the cuspatial repository, or the CUSPATIAL_ROOT environment variable must be set."
    else
        echo "Script failed with unknown error attempting to determine project root:"
        echo ${CUSPATIAL_BUILD_DIR}
    fi
    exit 1
fi

DEFAULT_FORMAT_FILE_LOCATIONS=(
  "${CUSPATIAL_BUILD_DIR:-${HOME}}/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
)
