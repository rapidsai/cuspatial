...
if (( ${NUMARGS} == 0 )) || hasArg cuspatial; then
    ...
    SKBUILD_CONFIGURE_OPTIONS="-DCMAKE_PREFIX_PATH=${INSTALL_PREFIX} -DCMAKE_LIBRARY_PATH=${LIBCUSPATIAL_BUILD_DIR} -DCMAKE_CUDA_ARCHITECTURES=${CUSPATIAL_CMAKE_CUDA_ARCHITECTURES} ${EXTRA_CMAKE_ARGS}" \
...
if (( ${NUMARGS} == 0 )) || hasArg cuproj; then
    ...
    SKBUILD_CONFIGURE_OPTIONS="-DCMAKE_PREFIX_PATH=${INSTALL_PREFIX} -DCMAKE_LIBRARY_PATH=${LIBCUPROJ_BUILD_DIR} -DCMAKE_CUDA_ARCHITECTURES=${CUSPATIAL_CMAKE_CUDA_ARCHITECTURES} ${EXTRA_CMAKE_ARGS}" \
        SKBUILD_BUILD_OPTIONS="-j${PARALLEL_LEVEL:-1}" \
        python -m pip install --no-build-isolation --no-deps .
fi
