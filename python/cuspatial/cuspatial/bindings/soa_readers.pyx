...
cpdef cpp_read_id_soa(soa_file_name):
...
cpdef cpp_read_pnt_soa(soa_file_name):
	print("in cpp_read_pnt_soa, reading ",soa_file_name)
	cdef bytes py_bytes = soa_file_name.encode()
	cdef char* c_string = py_bytes
	cdef gdf_column* c_pnt_x=<gdf_column*>malloc(sizeof(gdf_column))
	cdef gdf_column* c_pnt_y=<gdf_column*>malloc(sizeof(gdf_column))
	with nogil:
		read_pnt_soa(c_string,c_pnt_x[0],c_pnt_y[0])
