...
cpdef cpp_point_in_polygon_bitmap(
    ...
    bitmap = gdf_column_to_column(result_bitmap)
    free(c_points_x)
    free(c_points_y)
    free(c_poly_fpos)
    free(c_poly_rpos)
...
cpdef cpp_haversine_distance(x1, y1, x2, y2):
    ...
    h_dist = gdf_column_to_column(c_h_dist)
    free(c_x1)
    free(c_y1)
    free(c_x2)
    free(c_y2)
...
cpdef cpp_lonlat2coord(cam_lon, cam_lat, in_lon, in_lat):
    ...
    x = gdf_column_to_column(&coords.first)
    y = gdf_column_to_column(&coords.second)
...
cpdef cpp_directed_hausdorff_distance(coor_x, coor_y, cnt):
    ...
    dist = gdf_column_to_column(c_dist)
...
cpdef cpp_spatial_window_points(left, bottom, right, top, x, y):
    ...
    outx = gdf_column_to_column(&xy.first)
    outy = gdf_column_to_column(&xy.second)
