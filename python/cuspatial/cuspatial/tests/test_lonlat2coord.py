...
def test_camera_oob_0():
    with pytest.raises(RuntimeError):
        result = cuspatial.lonlat_to_xy_km_coordinates(  # noqa: F841
...
def test_longest_distance():
    result = cuspatial.lonlat_to_xy_km_coordinates(
        -180, -90, cudf.Series([180]), cudf.Series([90])
    )
...
def test_half_distance():
    result = cuspatial.lonlat_to_xy_km_coordinates(
        -180, -90, cudf.Series([0]), cudf.Series([0])
    )
    assert_eq(
...
def test_missing_coords():
    with pytest.raises(RuntimeError):
        result = cuspatial.lonlat_to_xy_km_coordinates(  # noqa: F841
            -180, -90, cudf.Series(), cudf.Series([0])
        )
