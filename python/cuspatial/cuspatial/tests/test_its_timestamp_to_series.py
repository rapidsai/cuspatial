...
def test_zero():
    assert_eq(
        its_timestamp_int64_to_datetime64ms(cudf.Series([0])._column),
...
def test_one():
    assert_eq(
        its_timestamp_int64_to_datetime64ms(cudf.Series([1])._column),
...
def test_zeroes():
    assert_eq(
        its_timestamp_int64_to_datetime64ms(cudf.Series([0, 0])._column),
...
def test_ones():
    assert_eq(
        its_timestamp_int64_to_datetime64ms(cudf.Series([1, 1])._column),
